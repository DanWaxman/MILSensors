...
parser = argparse.ArgumentParser(description="Minimum Preditive Variance (MPV)")
...
N_init = args.N_init  # number of initial points from June 2013 to train GP on
...
# %% FIT MODEL ON N_INIT=5 POINTS TO GET HYPERPARAMETERS; THEN COMPUTE POSTERIOR PREDICTIVE TRACE AFTER ADDING N_TO_ADD POINTS
...
####### Apply k means with N_init centroids and select the closest grid points
centroids = kmeans2(R_full[0, ...], N_init, minit="points")[0]
...
assert len(sample_points) == N_init, (
    "Number of initial training points does not match N_init"
)
...
X = X.reshape(24 * 30 * N_init, 3)
Y = Y.reshape(24 * 30 * N_init, 1)
...
# DUMMY MODEL FOR GETTING PREDICTIVE VARIANCE AND BEING ABLE TO CHANGE SOME OF THE TRAINING INPUTS WITHOUT CHANGING THE MODEL
N_t_for_dummy = t.shape[0]


def get_objective_imse(inputs_to_try):
    dummy_t = t
    dummy_R = jnp.stack(
        [
            jnp.concatenate([R[0], inputs_to_try.reshape(-1, 2)], axis=0)
            for _ in range(N_t_for_dummy)
        ],
        axis=0,
    )
    dummy_Y = jnp.zeros_like(dummy_R)[:, :, :1]
    dummy_model = MarkovGaussianProcess(
        model.kernel, model.likelihood, dummy_t, dummy_Y, dummy_R
    )
    # _, pvar = dummy_model.predict(jnp.arange(24*30, 2*24*30), R_t[:24*30])  # get predictive variance for the next 30 days (July 2013)
    _, pvar = dummy_model.predict(
        jnp.arange(24 * 30, 24 * 30 + 24 * 10), R_t[: 24 * 10]
    )  # get predictive variance for the next 10 days (July 2013)
    return jnp.sum(pvar) / pvar.shape[0]  # return the mean predictive variance
...
def pgd(
    ...
    for _ in pbar:
        ...
        pbar.set_description(f"Loss: {val:.6f}")
        loss_vals.append(val)
    return pts, loss_vals


centroids = kmeans2(R_full[0, ...], N_to_add, minit="points")[0]
...
np.savez(
    f"results/mpv_phoenix_N_init_{args.N_init}_N_added_{N_to_add}_seed_{seed}.npz",
    seed=seed,
